...
namespace WolfEyes
{
    public class Camera
    {
        ...
        private BackgroundSubtractorMOG2 mog = new BackgroundSubtractorMOG2(100, 5, false);
        private StructuringElementEx morph = new StructuringElementEx(3, 3, 1, 1, CV_ELEMENT_SHAPE.CV_SHAPE_ELLIPSE);
        private StructuringElementEx morph2 = new StructuringElementEx(10, 10, 0, 0, CV_ELEMENT_SHAPE.CV_SHAPE_ELLIPSE);
        ...
        public D2Point FingerPosition(Camera camera)
        {
            ...
            if (a != c)
            {
                ...
                clicking = (fingerPosition.y >= 0.999 && camera.fingerPosition.y >= 0.999);
        ...
        /*public Image<Bgr, byte> DetectHybrid(Image<Bgr, byte> frame, byte hueLowMin, byte hueLowMax, byte hueHighMin, int threshold, int diffThreshold, int dilate, int minArea)
        {
            try
            {
                int a = (int)(band.x * Height);
                ...
                StructuringElementEx morph = new StructuringElementEx(5, 5, 2, 2, CV_ELEMENT_SHAPE.CV_SHAPE_ELLIPSE);
                diff = diff.MorphologyEx(morph, CV_MORPH_OP.CV_MOP_OPEN, 1);

                MemStorage storage = new MemStorage();
                Image<Bgr, byte> output = diff.Convert<Bgr, byte>();
                Contour<Point> contours = diff.FindContours(CHAIN_APPROX_METHOD.CV_CHAIN_APPROX_SIMPLE, RETR_TYPE.CV_RETR_TREE);

                for (Contour<Point> contour = contours; contour != null; contour = contour.HNext)
                {
                    //Seq<Point> hull = contour.GetConvexHull(ORIENTATION.CV_CLOCKWISE, storage);
                    Seq<Point> hull = contour.ApproxPoly(contour.Perimeter * 0.005, storage);

                    if (hull.Area > 500)
                    {
                        output.Draw(hull, new Bgr(0, 0, 255), 10);

                        Seq<MCvConvexityDefect> defects = hull.GetConvexityDefacts(storage, ORIENTATION.CV_COUNTER_CLOCKWISE);

                        fingerPosition.x = defects.Total;

                        foreach(MCvConvexityDefect defect in defects)
                        {
                            output.Draw(new LineSegment2D(defect.StartPoint, defect.EndPoint), new Bgr(0, 255, 255), 10);
                            output.Draw(new CircleF(defect.DepthPoint, 5), new Bgr(0, 255, 0), 10);
                        }
                    }
        ...
        public Image<Bgr, byte> DetectHybrid(Image<Bgr, byte> frame, byte hueLowMin, byte hueLowMax, byte hueHighMin, int threshold, int diffThreshold, int dilate, int minArea)
        {
            try
            {
                ...
                for (Contour<Point> contour = contours; contour != null; contour = contour.HNext)
                {
                    ...
                    if (hull.Area > 500)
                    {
                        output.Draw(hull, new Bgr(0, 0, 255), 5);
                        ...
                        foreach (MCvConvexityDefect defect in defects)
                        {
                            Point vectA = new Point(defect.DepthPoint.X - defect.StartPoint.X, defect.DepthPoint.Y - defect.StartPoint.Y);
                            Point vectB = new Point(defect.DepthPoint.X - defect.EndPoint.X, defect.DepthPoint.Y - defect.EndPoint.Y);

                            double magnA = Math.Sqrt(vectA.X * vectA.X + vectA.Y * vectA.Y);
                            double magnB = Math.Sqrt(vectB.X * vectB.X + vectB.Y * vectB.Y);

                            double angle = Math.Acos((vectA.X * vectB.X + vectA.Y * vectB.Y) / (magnA * magnB));

                            if(angle < 120 * Math.PI / 180)
                            {
                                output.Draw(new CircleF(defect.DepthPoint, 5), new Bgr(0, 255, 0), 5);
                                output.Draw(new CircleF(defect.EndPoint, 5), new Bgr(0, 255, 255), 5);
                                fingerPosition = new D2Point(defect.EndPoint.X, defect.EndPoint.Y);
                                //output.Draw(new LineSegment2D(defect.StartPoint, defect.EndPoint), new Bgr(0, 255, 255), 5);
                            }
                        }
                    }
            ...
            try
            {
                ...
                for (Contour<Point> contour = contours; contour != null; contour = contour.HNext)
                {
                    ...
                    if (hull.Area > 500)
                    {
                        ...
                        int maxY = hull.Max(element => element.Y);
                        if(maxY > fingerPosition.y) fingerPosition.y = (maxY + 2.0) / frame.Height;

                        int tempMinX = hull.Min(element => element.X);
                        if (tempMinX < minX) minX = tempMinX;

                        int tempMaxX = hull.Max(element => element.X);
