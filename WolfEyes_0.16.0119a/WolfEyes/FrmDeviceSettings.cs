...
namespace WolfEyes
{
    public partial class FrmDeviceSettings : Form
    {
        ...
        FrmDebug test = new FrmDebug();

        public FrmDeviceSettings(bool editing)
        {
            InitializeComponent();
        ...
        private void Hook_KeyDown(object sender, KeyEventArgs e)
        {
            if(e.KeyCode == Keys.F1)
            {
                foreach (Camera device in ApplicationData.selectedPreset.Devices)
                    new Task(() => device.CumulativeReference(50)).Start();
            }
        }

        private void UpdateEngine()
        {
            Engine.SetCameraOutput(0, ApplicationData.selectedPreset.DetectionMethod, test.pictureBox1, true);
            //Engine.SetCameraOutput(1, ApplicationData.selectedPreset.DetectionMethod, picDevice2);
            Engine.SetFingerInfoOutput(label6, 0);
            Engine.StartEngine();
        }
