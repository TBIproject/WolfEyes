...
namespace WolfEyes
{
    public class Camera
    {
        ...
        public Image<Bgr, byte> DetectHybrid(Image<Bgr, byte> frame, byte hueLowMin, byte hueLowMax, byte hueHighMin, int threshold, int diffThreshold, int dilate, int minArea)
        {
            ...
            Image<Gray, byte> result = (h / 2 + a / 2);
            result = Threshold(result, threshold);

            result = (result / 2 + diff / 2);
            result = Threshold(result, dilate);

            //CvInvoke.cvFilter2D(result, )

            Image<Bgr, byte> output = result.Convert<Bgr, byte>();
            Contour<Point> contours = result.FindContours(Emgu.CV.CvEnum.CHAIN_APPROX_METHOD.CV_CHAIN_APPROX_SIMPLE, Emgu.CV.CvEnum.RETR_TYPE.CV_RETR_TREE);
