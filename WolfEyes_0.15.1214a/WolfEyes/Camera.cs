    ...
    public class Camera
    {
        ...
        public D2Point FingerPosition(Camera camera)
        {
            ...
            if (a != c)
            {
                ...
                clicking = (fingerPosition.y > 0.95 && camera.fingerPosition.y > 0.95);
        ...
        /*public Image<Bgr, byte> DetectByBlob(Image<Bgr, byte> frame)
        {
            try
            {
                SimpleBlobDetector blob = new SimpleBlobDetector();
                MKeyPoint[] points = blob.Detect(frame);
                Features2DToolbox.DrawKeypoints(frame, new VectorOfKeyPoint(points), frame, new Bgr(0, 0, 255), Features2DToolbox.KeypointDrawType.DrawRichKeypoints);
            }
            catch
            {
                return null;
